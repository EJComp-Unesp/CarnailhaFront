...
class Partners extends React.Component {
    ...
    _load = async () => {
        ...
        if (response.data.code === 200) {
            ...
            response.data.partners.map(partner => {
                ac.push(createData(partner.name, partner._id));
            })
            this.setState({ rows: ac, partners: response.data.partners });
        } else {
            alert("Um erro ocorreu");
        }
    };
    _save = async () => {
        const { partners, name, description, link, partner, file, rows } = this.state;
        ...
        if (response.data.code != 201) {
            ...
            types = "success";
            partners.push(response.data.partner);
            rows.push(createData(response.data.partner.name, response.data.partner._id))
            this.setState({ partners, rows });
        }
    ...
    _update = async () => {
        const { partners, rows, id, _id, name, description, link, partner, file } = this.state;
        ...
        if (response.data.code != 200) {
            ...
            let r = rows;
            let count = id - 1;
            acc[count] = response.data.partner;
            r[count] = { id: id, name: acc[count].name, _id: acc[count]._id }
            this.setState({ partners: acc, rows: r });
        }
    ...
    _delete = async () => {
        const { partners, rows, id, _id } = this.state;
        const data = {
            _id: _id
        };
    ...
    handleClearFields = () => {
        const name = "",
            ...
            description = "",
            link = ""
        this.setState({ name, partner, img, description, link });
    };
                ...
                <GridItem xs={12} sm={12} md={12}>
                    <Card>
                        <CardHeader color="primary">
                            <h4 className={classes.cardTitleWhite}>{(this.state.action == 1) ? `Novo Parceiro` : `Editar Parceiro`}</h4>
                        </CardHeader>
                        <CardBody>
                  ...
                  {this.state.states.map((option, index) => (
                    <option key={index} value={option.Nome}>
                      {option.Nome}
                        ...
                        <CardFooter>
                            <GridItem xs={12} sm={12} md={12}>
                                <p>Parceiros já cadastrados:</p>
                                <ul>
                                    {this.state.partners.map(name => (
                                ...
                                <TableRow key={row.id}>
                                    ...
                                    <TableCell align="right">
                                        <IconButton aria-label="Edit" onClick={() => {
                                            let ac = this.state.partners[row.id - 1];
                                            console.log(row.id);
                                            this.setState({
                                                action: 2,
                                                id: row.id,
                    ...
                    <Card>
                        ...
                        <CardHeader color="primary">
                            <h4 className={classes.cardTitleWhite}>
                                Parceiros Cadastradas
                </h4>
                            <p>
                                Área de gerenciamento dos parceiros do site. Você pode inserir
                                um novo parceiro e editar os parceiros já existentes.
                </p>
                            <Button
                                onClick={() => {
                                    this.setState({ action: 1 });
